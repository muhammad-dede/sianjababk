<form action="{{ route('skpd.update', $skpd->id) }}" class="modal-form" method="POST">
    @csrf
    @method('put')
    ...
    <div class="modal-body">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="nama" id="nama" value="{{ $skpd->nama }}">
        </div>
        <div class="mb-3">
            ...
            <select class="form-select" name="id_unit_kerja" id="id_unit_kerja">
                ...
                @foreach (unitKerja() as $row)
                    <option value="{{ $row->id }}" {{ $skpd->id_unit_kerja == $row->id ? 'selected' : '' }}>
                        {{ $row->nama }}
                    </option>
                @endforeach
            </select>
